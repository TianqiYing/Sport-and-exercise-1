...
def tune_hyperparams(train_df, pos, features, n_trials=50, val_seasons=2):
    ...
    def objective(trial):
        ...
        return rank_weighted_mae(y_val, preds) # Change to mae or RW-MAE
...
def compare_season_performance(df, start_gw, end_gw, season, budget=83.0, export=True):
    ...
    plt.tight_layout()
    plt.savefig(f"Data/xgbplot1_pred_vs_opt_{season}.png", dpi=150)
    plt.show()
    ...
    plt.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"Data/xgbplot3_recall_{season}.png", dpi=150)
    plt.show()
...
def optimal_team_predictability(df, start_gw, end_gw, season, budget=83.0, export=True):
    ...
    plt.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"Data/xgbplot4_gap_vs_expected_{season}.png", dpi=150)
    plt.show()
    ...
    plt.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"Data/xgbplot5_surprise_{season}.png", dpi=150)
    plt.show()
    ...
    plt.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"Data/xgbplot6_rank_{season}.png", dpi=150)
    plt.show()
