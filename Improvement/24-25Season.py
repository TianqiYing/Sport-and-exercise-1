...
def plot_season_figures(
    ...
    if "name" in df.columns:
        ...
        star = df[df["name"].astype(str).str.contains(highlight_name_substr, case=False, na=False)]
        if len(star) > 0:
            r = star.iloc[0]
            plt.scatter(
                [float(r["mean_std_points"])],
                [float(r["mean_expected_points"])],
                s=140,
                facecolors="none",
                edgecolors="red",
                linewidths=2.2,
...
def main():
    ...
    if len(gws) >= 2:
        ...
        # We accumulate per-player expected/std/price and selection frequency over the whole season.
